...
function createPolicySection(title, simulationData, metrics, explanation, policyIndex) {
    ...
    header.innerHTML = `
        ...
        <div class="metrics">
            ...
            <div class="metric">Ciclos: ${Math.max(...simulationData.map(inst => inst.length))}</div>
            <div class="metric">Eficiencia: ${metrics.resourceUtilization}</div>
        </div>
    `;
    ...
    for (let cycle = 1; cycle <= maxCycles; cycle++) {
        ...
        simulationState.currentConfig.forEach((inst, index) => {
            ...
            if (cycleData) {
                ...
                // Resaltar diferencias entre políticas
                if (policyIndex > 0) {
                    const prevPolicyData = policyIndex === 1 ? 
                        simulationState.policy1Data[index][cycle - 1] :
                        simulationState.policy2Data[index][cycle - 1];
                    
                    if (prevPolicyData && cycleData.stage !== prevPolicyData.stage) {
                        cellClass += ' raw-dependency'; // Usar color diferente para resaltar
                    }
                }
            }
    ...
    // Añadir análisis específico de la política
    const analysisDiv = document.createElement('div');
    analysisDiv.className = 'explanation';
    
    const totalCycles = Math.max(...simulationData.map(inst => inst.length));
    const instructionCount = simulationState.currentConfig.length;
    const ipc = (instructionCount / totalCycles).toFixed(2);
    
    let analysisText = explanation + `<br><br>`;
    analysisText += `<strong>Análisis específico:</strong><br>`;
    analysisText += `• Ciclos totales: ${totalCycles}<br>`;
    analysisText += `• Instrucciones completadas: ${instructionCount}<br>`;
    analysisText += `• IPC: ${ipc}<br>`;
    
    if (policyIndex === 1) {
        const cyclesPolicy1 = Math.max(...simulationState.policy1Data.map(inst => inst.length));
        analysisText += `• Mejora respecto a Política 1: ${((cyclesPolicy1 - totalCycles) / cyclesPolicy1 * 100).toFixed(1)}% más rápido<br>`;
    } else if (policyIndex === 2) {
        const cyclesPolicy2 = Math.max(...simulationState.policy2Data.map(inst => inst.length));
        analysisText += `• Mejora respecto a Política 2: ${((cyclesPolicy2 - totalCycles) / cyclesPolicy2 * 100).toFixed(1)}% más rápido<br>`;
    }
...
function simulateInOrderOutOfOrder(config) {
    const simulationData = config.map(() => []);
    const status = config.map((inst, idx) => ({
        index: idx,
        decodeCycle: 0,
        executeStartCycle: 0,
        writeCycle: 0,
    ...
    // Inicializar colas por tipo de unidad
    Object.keys(simulatorConfig.functionalUnits).forEach(unit => {
        unitQueues[unit] = {
            busyUntil: 0,
            instructions: []
        };
    ...
    while (issuedIndex < config.length || readyQueue.length > 0 || writeQueue.length > 0) {
        // === FASE 1: EMISIÓN (en orden) ===
        if (issuedIndex < config.length) {
            const instruction = config[issuedIndex];
            const instructionStatus = status[issuedIndex];
            
            // Emitir instrucción
            instructionStatus.decodeCycle = currentCycle;
            
            // Verificar dependencias
            const rawDependenciesSatisfied = instruction.dependencies.every(depId => {
                const depIdx = config.findIndex(inst => inst.id === depId);
                return depIdx === -1 || status[depIdx].writeCycle > 0;
            });
            
            instructionStatus.dependenciesSatisfied = rawDependenciesSatisfied;
            
            if (rawDependenciesSatisfied) {
                instructionStatus.readyToExecute = true;
                readyQueue.push(instructionStatus);
            }
            
            issuedIndex++;
        }
        
        // === FASE 2: EJECUCIÓN (fuera de orden) ===
        // Procesar cola de instrucciones listas para ejecutar
        const executedThisCycle = [];
        
        for (let i = readyQueue.length - 1; i >= 0; i--) {
            const instructionStatus = readyQueue[i];
            const instruction = config[instructionStatus.index];
            const unitType = instruction.functionalUnit;
            const unitQueue = unitQueues[unitType];
            
            // Verificar si la unidad está disponible
            if (unitQueue.busyUntil < currentCycle) {
                // Iniciar ejecución
                instructionStatus.executeStartCycle = currentCycle;
                instructionStatus.readyToExecute = false;
                instructionStatus.readyToWrite = true;
                
                // Programar finalización de ejecución
                const executionEnd = currentCycle + instruction.executionTime - 1;
                unitQueue.busyUntil = executionEnd;
                
                // Mover a cola de escritura para después de ejecutar
                writeQueue.push({
                    instructionStatus: instructionStatus,
                    readyCycle: executionEnd + 1
                });
                
                // Remover de cola de listas
                readyQueue.splice(i, 1);
                executedThisCycle.push(instructionStatus);
                
                // Verificar límite de ancho de emisión
                if (executedThisCycle.length >= simulatorConfig.issueWidth) {
                    break;
                }
            }
        }
        
        // === FASE 3: ESCRITURA (fuera de orden) ===
        // Procesar instrucciones que terminaron ejecución
        for (let i = writeQueue.length - 1; i >= 0; i--) {
            const item = writeQueue[i];
            
            if (currentCycle >= item.readyCycle) {
                const instructionStatus = item.instructionStatus;
                
                // Verificar dependencias WAR/WAW (si no hay renombrado)
                if (!simulationState.registerRenaming) {
                    const instruction = config[instructionStatus.index];
                    const warConflict = instruction.war?.some(depId => {
                        const depIdx = config.findIndex(inst => inst.id === depId);
                        return depIdx !== -1 && 
                               status[depIdx].executeStartCycle > 0 &&
                               status[depIdx].executeStartCycle <= currentCycle &&
                               status[depIdx].writeCycle === 0;
                    });
                    
                    const wawConflict = instruction.waw?.some(depId => {
                        const depIdx = config.findIndex(inst => inst.id === depId);
                        return depIdx !== -1 && status[depIdx].writeCycle === 0;
                    });
                    
                    if (warConflict || wawConflict) {
                        continue; // Esperar otro ciclo
                    }
                }
                
                // Escribir resultados
                instructionStatus.writeCycle = currentCycle;
                instructionStatus.completed = true;
                instructionStatus.readyToWrite = false;
                
                // Remover de cola
                writeQueue.splice(i, 1);
                
                // Activar instrucciones que dependían de esta
                config.forEach((inst, idx) => {
                    if (inst.dependencies.includes(config[instructionStatus.index].id) &&
                        !status[idx].readyToExecute && 
                        !status[idx].executeStartCycle) {
                        
                        // Verificar si todas las dependencias están satisfechas
                        const allDepsSatisfied = inst.dependencies.every(depId => {
                            const depIdx = config.findIndex(i => i.id === depId);
                            return depIdx === -1 || status[depIdx].writeCycle > 0;
                        });
                        
                        if (allDepsSatisfied) {
                            status[idx].readyToExecute = true;
                            status[idx].dependenciesSatisfied = true;
                            readyQueue.push(status[idx]);
                        }
                    }
                });
            }
    ...
    config.forEach((instruction, idx) => {
        const s = status[idx];
        
        if (s.decodeCycle > 0) {
            simulationData[idx][s.decodeCycle - 1] = {
                stage: 'decode',
                text: 'D'
            };
        }
        
        if (s.executeStartCycle > 0) {
            for (let i = 0; i < instruction.executionTime; i++) {
                simulationData[idx][s.executeStartCycle - 1 + i] = {
                    stage: 'execute',
                    text: `E${i+1}`
                };
            }
        ...
        if (s.writeCycle > 0) {
            simulationData[idx][s.writeCycle - 1] = {
                stage: 'write',
                text: 'W'
            };
        }
...
function simulateOutOfOrderOutOfOrder(config) {
    ...
    // Inicializar: instrucciones sin dependencias están listas para emitir
    config.forEach((instruction, idx) => {
        if (instruction.dependencies.length === 0) {
            status[idx].readyToIssue = true;
            issueQueue.push(status[idx]);
        }
    });
    ...
    while (issueQueue.length > 0 || readyQueue.length > 0 || writeQueue.length > 0) {
        ...
        // Emitir hasta issueWidth instrucciones por ciclo
        for (let i = issueQueue.length - 1; i >= 0; i--) {
            if (issuedThisCycle.length >= simulatorConfig.issueWidth) break;
            
            const instructionStatus = issueQueue[i];
            
            // Emitir instrucción
            instructionStatus.decodeCycle = currentCycle;
            instructionStatus.issued = true;
            instructionStatus.readyToIssue = false;
            ...
            issuedThisCycle.push(instructionStatus);
            issueQueue.splice(i, 1);
        }
        
        // === FASE 2: EJECUCIÓN DESORDENADA ===
        const executedThisCycle = [];
        
        // Intentar ejecutar instrucciones listas
        for (let i = readyQueue.length - 1; i >= 0; i--) {
            if (executedThisCycle.length >= simulatorConfig.issueWidth) break;
        ...
        // Procesar instrucciones listas para escribir
        for (let i = writeQueue.length - 1; i >= 0; i--) {
            const item = writeQueue[i];
            ...
            if (currentCycle >= item.readyCycle) {
                ...
                if (canWrite) {
                    ...
                    // Activar instrucciones que dependían de esta
                    config.forEach((inst, idx) => {
                        if (!status[idx].issued && inst.dependencies.includes(instruction.id)) {
                            // Verificar si todas las dependencias están satisfechas
                            const allDepsSatisfied = inst.dependencies.every(depId => {
                                const depIdx = config.findIndex(i => i.id === depId);
                                return depIdx === -1 || status[depIdx].writeCycle > 0;
                            ...
                            if (allDepsSatisfied && !status[idx].readyToIssue) {
                                status[idx].readyToIssue = true;
                                issueQueue.push(status[idx]);
                            }
                        }
                        
                        // Para instrucciones ya emitidas pero esperando
                        if (status[idx].issued && !status[idx].readyToExecute) {
                            const depsSatisfied = inst.dependencies.every(depId => {
                                const depIdx = config.findIndex(i => i.id === depId);
                                return depIdx === -1 || status[depIdx].writeCycle > 0;
                            });
                            
                            if (depsSatisfied && !status[idx].executeStartCycle) {
                                status[idx].readyToExecute = true;
                                readyQueue.push(status[idx]);
                            }
                        }
        ...
        // === FASE 4: ACTUALIZAR COLAS DE EMISIÓN ===
        // Añadir nuevas instrucciones listas para emitir
        config.forEach((instruction, idx) => {
            if (!status[idx].issued && !status[idx].readyToIssue) {
                const allDepsSatisfied = instruction.dependencies.every(depId => {
                    const depIdx = config.findIndex(inst => inst.id === depId);
                    return depIdx === -1 || status[depIdx].writeCycle > 0;
                });
    ...
    config.forEach((instruction, idx) => {
        ...
        if (s.decodeCycle > 0) {
            simulationData[idx][s.decodeCycle - 1] = {
                stage: 'decode',
                text: 'D',
                conflict: s.decodeCycle > 1 && !s.readyToIssue
            };
        }
        ...
        if (s.executeStartCycle > 0) {
            for (let i = 0; i < instruction.executionTime; i++) {
                simulationData[idx][s.executeStartCycle - 1 + i] = {
                    stage: 'execute',
                    text: `E${i+1}`,
                    dependency: i === 0 && !s.readyToExecute
                };
            }
        }
        ...
        if (s.writeCycle > 0) {
            simulationData[idx][s.writeCycle - 1] = {
                stage: 'write',
                text: 'W',
                conflict: s.writeCycle > s.executeStartCycle + instruction.executionTime
            };
        }
    });
